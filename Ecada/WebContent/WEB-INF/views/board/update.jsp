...
<script type="text/javascript">
	...
	function doUpdate(){
	    ...
	    var ajaxReq = $.ajax({
	    	url : "<c:url value='/free/update.do' />",
	          data : formData,
	          type : 'POST',
	          dataType : "text",
	          processData : false,
	...
	function doDelAtt(){
		//href="<c:url value='/free/deleteAttach.do
		...
		//$('#btnDelAtt').on('click', function() {
				var ajaxReq = $.ajax({
			    	url : "<c:url value='/free/deleteAttach.do' />",
			        ...
			        success : function (result, textStatus, XMLHttpRequest) {
			              ...
			              if(result == 1){
			                  //alert("게시물이 삭제되었습니다.");
			                  //movePage(this, "/free/update.do?typeSeq=2", "pageName=free");
		...
		<div class="container">
			<div class="row">
				...
				<div class="col-md-12">
					...
					<input type="hidden" name="typeSeq" id="typeSeq" value="2"/>
					<input type="hidden" name="boardSeq" id="boardSeq" value="${ board.board_seq }"/>
					<input type="hidden" name="hasFile" id="hasFile" value="${ board.has_file }"/>
					...
					<input type="hidden" name="memberIdx" value="${sessionScope.memberIdx}"/>
 							...
 							<c:choose>
									<c:when test="${ !empty files }">
										...
										<input type="hidden" name="fileIdx" id="fileIdx" value="${ file.file_idx }"/>										
											...
											<%-- href="<c:url value='/free/deleteAttach.do?fileIdx=${ file.file_idx }&amp;boardSeq=${ board.board_seq }&amp;typeSeq=${ board.type_seq }'/>" --%>
						...
						<td align="right"><a href="javascript:pageMove(this, '/free/fList.do?typeSeq=2')"><button type="button" class="btn-e btn-e-primary">목록</button></a></td>
						</tr>
						</table>
						</div>
					</form>
